...
#include "RotationBesselSurface.h"
#include "tools.h"
#include "easylogging++\easylogging++.h"

RotationBesselSurface::RotationBesselSurface(const BesselCurve& bessel_curve,
	...
	v_(0.0)
{
	for (const auto& c : bessel_curve.ctrl_points_)
	{
		bessel_curve_.push_ctrl_points(c);
	}
	// set bounding box
}
...
bool RotationBesselSurface::hit(const Ray & ray, float t0, float t1, float & t)
{
	//bounding box check
...
Point3F RotationBesselSurface::hit_normal(const Ray & ray, float t) const
{
...
QPointF RotationBesselSurface::pu_(double u) const
{
	int n = bessel_curve_.ctrl_points_size() - 1;
	QPointF p(0.0, 0.0);
	for (int i = 0; i <= n; ++i)
	{
		p += bessel_curve_.ctrl_points_[i] * BernsteinBaseFunc(i, n, u);
	}
	return p;
}

QPointF RotationBesselSurface::dpu_(double u) const
{
	int n = bessel_curve_.ctrl_points_size() - 1;
	QPointF dp(0.0, 0.0);
	for (int i = 0; i < n; ++i)
	{
		dp += bessel_curve_.ctrl_points_[i]
